 ...
 * 
 * <p>
 * This class takes care of opening/closing the connection to the server.
 * Opening the connection includes setting up the XML streams, upgrading to TLS,
 * authentication, and resource binding.
 * 
 * <p>
 * You are NOT required to understand the below code for this assignment. Simply
 * use this class to open/close the connection, and get the resulting socket.
 * 
 * <p>
 ...
 * <li>Use the {@link #connect()} method to open a connection to the server.</li>
 * <li>You can get the connected socket using the {@link #getSocket()} method,
 * and associated readers and writers using the methods {@link #getReader()} and
 * {@link #getWriter()} for I/O. You can use the XML pull-parser I use for
 * parsing XML. Get the parser using {@link #getParser()}. If you are not sure
 * how to go about parsing XML, then peek into the code in this class for
 * examples.</li>
 * <li>You may require the stream ID (provided by the server during connection
 * setup). You can get this using the {@link #getStreamID()} method.</li>
 * <li>Finally, after you are done, close the connection using the
 * {@link #close()} method.</li>
 * </ol>
 */
class XmppConnection {
	...
	public void connect() throws IOException {
		...
		System.out.println("Connecting to " + jid.getServerName() + " at port "
				+ jid.getServerPort());
		socket.connect(
				new InetSocketAddress(jid.getServerName(), jid.getServerPort()),
				connectionTimeout);
		System.out.println("Connected to " + jid.getServerName() + " at port "
				+ jid.getServerPort());
	...
	public void sendMessage(String receiver, String content) throws IOException {
		...
		if (writer != null) {
			...
			JabberMain.unsendedMessages.add(content);
		}
	...
	private void createXmlStream() throws IOException {
	...
	private void createXmlParser() throws XMLStreamException {
	...
	private boolean handleServerStream() throws IOException, XMLStreamException {
	...
	private void handleStreamTag() throws IOException {
	...
	private void handleFeaturesTag() throws IOException, XMLStreamException {
	...
	private void handleMechanismsTag() throws IOException, XMLStreamException {
	...
	private void upgradeToTls() throws IOException {
		...
		SSLSocket sslSocket = (SSLSocket) factory.createSocket(socket, // Old
																		// Socket
	...
	private void authenticate() throws IOException {
		...
		// At this point of time, Google supports PLAIN and Facebook supports
		// DIGEST-MD5
		// Note: PLAIN Password is still secure, since both Google and Facebook
		// support TLS
		...
		sc = Sasl.createSaslClient(mechanismArray, // Mechanisms supported by
													// server
	...
	private String createAuthenticationStanza(String authName, byte[] response) {
	...
	private void respondToChallenge() throws IOException, XMLStreamException {
	...
	private void handleFailureTag() throws IOException, XMLStreamException {
		...
		if (namespace == null) {
			...
			throw new IOException(
					"Server sent Failure stanza: Authentication failure");
		} else if (namespace.equals("urn:ietf:params:xml:ns:xmpp-tls")) {
			throw new IOException(
					"Server sent Failure stanza: Unable to upgrade to TLS");
		} else {
	...
	private class JabberCallbackHandler implements CallbackHandler {
	...
	/** Handles the IQ tag. */
	private void handleIQTag() throws IOException, XMLStreamException {
	 ...
	 * System.out.println( "Handling server's IQ Query tag" ); boolean done =
	 * false;
	 * 
	 * while ( !done ) {
	 * 
	 * // Get the next XML event from the parser int eventType = parser.next();
	 * System.out.println("IQ query: "+eventType); // Check if the parse event
	 * is a XML start tag if ( eventType == XMLStreamConstants.START_ELEMENT ) {
	 * 
	 * // Case 1: mechanism (without 's') tag if ( parser.getLocalName().equals(
	 * "item" ) ) { // Add the authentication mechanism to the list
	 * contactList.add( parser.getAttributeValue(0) ); } } else if ( eventType
	 * == XMLStreamConstants.END_ELEMENT ) { if ( parser.getLocalName().equals(
	 * "query" ) ) { done = true; } } }
	 * 
	 * }
	 */
	/** Handles the IQ Bind tag. */
	private void handleIQBindTag() throws IOException, XMLStreamException {
		...
		while (!done) {
			...
			if (eventType == XMLStreamConstants.START_ELEMENT) {
				...
				if (parser.getLocalName().equals("jid")) {
					...
					if (split.length != 2) {
						throw new IOException(
								"Ill-formatted JID sent by server: " + fullJID);
					}
					jid.setResource(split[1]);

					// DEBUG
					System.out.println("Obtained Resource ID: "
							+ jid.getJabberID() + "/" + jid.getResource());
				}
			} else if (eventType == XMLStreamConstants.END_ELEMENT) {
				if (parser.getLocalName().equals("bind")) {
					done = true;
